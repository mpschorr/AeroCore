...
@Command("gmc")
@Permission("aerocore.command.gmc")
class GMCCommand(plugin: AeroCore) : BaseCommand() {
    val plugin: AeroCore
    init {
        this.plugin = plugin
    }
    ...
    fun gamemodeCommand(sender: CommandSender, @Optional @ArgName("target") cmdTarget: Player?) {
        ...
        if (target != sender) {
            sender.sendMessage(plugin.formatted(plugin.config.features.gamemode.messages.set_other.format(target.name, "creative")))
        }
        target.sendMessage(plugin.formatted(plugin.config.features.gamemode.messages.set_self.format("creative")))
